...
describe('ComentariosService - autorización de lectura de comentarios', () => {
  ...
  describe('findByTarea', () => {
    ...
    it('permite leer al líder y conserva filtro/include/orden', async () => {
      const prisma = makePrisma();
      prisma.tarea.findUnique.mockResolvedValue({ idProyecto: ID_PROYECTO });
      prisma.proyecto.findUnique.mockResolvedValue({ creadoPor: LIDER_ID });
      prisma.comentario.findMany.mockResolvedValue([{ idComentario: 2 }]);
      const service = new ComentariosService(prisma, {} as any);

      const result = await service.findByTarea(ID_TAREA, LIDER_ID);

      expect(result).toEqual([{ idComentario: 2 }]);
      expect(prisma.comentario.findMany).toHaveBeenCalledWith({
        where: { idTarea: ID_TAREA, eliminadoEn: null },
        include: { autor: AUTOR_SELECT },
        orderBy: { creadoEn: 'asc' },
      });
    });

    it('permite leer a un participante activo del proyecto real de la tarea', async () => {
      const prisma = makePrisma();
      prisma.tarea.findUnique.mockResolvedValue({ idProyecto: ID_PROYECTO });
      prisma.proyecto.findUnique.mockResolvedValue({ creadoPor: LIDER_ID });
      ...
      await service.findByTarea(ID_TAREA, PARTICIPANTE_ACTIVO_ID);
    ...
    it('rechaza a un usuario externo', async () => {
      ...
      await expect(service.findByTarea(ID_TAREA, EXTERNO_ID)).rejects.toBeInstanceOf(
        ForbiddenException,
      );
      expect(prisma.comentario.findMany).not.toHaveBeenCalled();
    });
    ...
    it('rechaza a un participante RETIRADO', async () => {
      ...
      await expect(service.findByTarea(ID_TAREA, 3)).rejects.toBeInstanceOf(ForbiddenException);
      expect(prisma.comentario.findMany).not.toHaveBeenCalled();
    });

    it('lanza NotFoundException si la tarea no existe y no continúa con la autorización', async () => {
      const prisma = makePrisma();
      prisma.tarea.findUnique.mockResolvedValue(null);
      const service = new ComentariosService(prisma, {} as any);

      await expect(service.findByTarea(999, LIDER_ID)).rejects.toBeInstanceOf(NotFoundException);
      expect(prisma.proyecto.findUnique).not.toHaveBeenCalled();
      expect(prisma.comentario.findMany).not.toHaveBeenCalled();
    });

    it('autoriza usando el idProyecto real obtenido de la tarea, no uno externo', async () => {
      const prisma = makePrisma();
      prisma.tarea.findUnique.mockResolvedValue({ idProyecto: OTRO_PROYECTO_ID });
      prisma.proyecto.findUnique.mockResolvedValue({ creadoPor: LIDER_ID });
      prisma.comentario.findMany.mockResolvedValue([]);
      const service = new ComentariosService(prisma, {} as any);

      await service.findByTarea(ID_TAREA, LIDER_ID);
    ...
    it('rechaza a un participante activo de otro proyecto cuando la tarea pertenece a un proyecto distinto', async () => {
      const prisma = makePrisma();
      // El usuario participa activamente en ID_PROYECTO, pero la tarea consultada
      // pertenece realmente a OTRO_PROYECTO_ID; su membresía en ID_PROYECTO no debe filtrarse.
      prisma.tarea.findUnique.mockResolvedValue({ idProyecto: OTRO_PROYECTO_ID });
      ...
      await expect(
        service.findByTarea(ID_TAREA, PARTICIPANTE_ACTIVO_ID),
  ...
  describe('findByTareaDesc', () => {
    const ID_TAREA = 20;

    it('permite leer al líder y conserva filtro/include/orden descendente', async () => {
      const prisma = makePrisma();
      prisma.tarea.findUnique.mockResolvedValue({ idProyecto: ID_PROYECTO });
      prisma.proyecto.findUnique.mockResolvedValue({ creadoPor: LIDER_ID });
      prisma.comentario.findMany.mockResolvedValue([{ idComentario: 3 }]);
      const service = new ComentariosService(prisma, {} as any);

      const result = await service.findByTareaDesc(ID_TAREA, LIDER_ID);
    ...
    it('permite leer a un participante activo', async () => {
      const prisma = makePrisma();
      prisma.tarea.findUnique.mockResolvedValue({ idProyecto: ID_PROYECTO });
      prisma.proyecto.findUnique.mockResolvedValue({ creadoPor: LIDER_ID });
      prisma.participacionProyecto.findFirst.mockResolvedValue({ idParticipacion: 1 });
      ...
      await service.findByTareaDesc(ID_TAREA, PARTICIPANTE_ACTIVO_ID);

      expect(prisma.comentario.findMany).toHaveBeenCalledTimes(1);
    });

    it('rechaza a un usuario externo (misma autorización que findByTarea, sin vía alterna)', async () => {
      const prisma = makePrisma();
      prisma.tarea.findUnique.mockResolvedValue({ idProyecto: ID_PROYECTO });
      prisma.proyecto.findUnique.mockResolvedValue({ creadoPor: LIDER_ID });
      prisma.participacionProyecto.findFirst.mockResolvedValue(null);
      ...
      await expect(service.findByTareaDesc(ID_TAREA, EXTERNO_ID)).rejects.toBeInstanceOf(
        ForbiddenException,
      );
      expect(prisma.comentario.findMany).not.toHaveBeenCalled();
    });

    it('rechaza a un participante COMPLETADO', async () => {
      const prisma = makePrisma();
      prisma.tarea.findUnique.mockResolvedValue({ idProyecto: ID_PROYECTO });
      prisma.proyecto.findUnique.mockResolvedValue({ creadoPor: LIDER_ID });
      prisma.participacionProyecto.findFirst.mockResolvedValue(null);
      ...
      await expect(service.findByTareaDesc(ID_TAREA, 4)).rejects.toBeInstanceOf(
        ForbiddenException,
      );
      expect(prisma.comentario.findMany).not.toHaveBeenCalled();
    });
    ...
    it('lanza NotFoundException si la tarea no existe', async () => {
      ...
      await expect(service.findByTareaDesc(999, LIDER_ID)).rejects.toBeInstanceOf(
        NotFoundException,
      );
      expect(prisma.proyecto.findUnique).not.toHaveBeenCalled();
      expect(prisma.comentario.findMany).not.toHaveBeenCalled();
